...
import nz.ac.massey.cs.care.move.refactoring.constraints.MyCompiler;
import nz.ac.massey.cs.care.move.refactoring.views.CareView;
import nz.ac.massey.cs.care.move.scripts.MoveExecuter;
import nz.ac.massey.cs.care.move.*;
import nz.ac.massey.cs.care.move.util.*;
...
public class MoveHelper {
	...
	private static MoveCandidate candidate = new MoveCandidate();
	...
	public static boolean applyMoveRefactoring(Edge winner,
			DirectedGraph<Vertex, Edge> g, Motif<Vertex, Edge>scdMotif, Motif<Vertex,Edge>wcdMotif, int totalInstances, IWorkbenchPartSite iwps) {
		iWorkbenchPartSite = iwps;
//		boolean succeeded = executeWinner(winner, totalInstances, new ArrayListmotifs, g)
//		if(succeeded){
...
//		}
	...
	private static boolean executeWinner(Edge winner, int initialInstances,
		...
		int moveSrcInstances = countAllInstances(g, motifs).getNumberOfInstances();//getResults(g, motifs).getNumberOfInstances();
		// reset graph
		...
		int moveTgtInstances = countAllInstances(g, motifs).getNumberOfInstances();//getResults(g, motifs).getNumberOfInstances();
		// reset graph
		resetGraph(classesMoved, oldNS);
		classesMoved.clear();
	...
	protected static void executeMoveRefactoring(MoveCandidate candidate, List<Motif<Vertex, Edge>> motifs, int totalInstances, boolean succeeded ) {
		IJavaProject p = ASTReader.getExaminedProject();
		ClassObject co = null;//candidate.getClassObjectToMove();
		if(co == null) return;
