...
/**
 * This class examines the AST of a class to determine class level dependencies
 * and whether several preconditions fail or not.
 * @author Ali Shah
 */
...
public class Checker {
	...
	protected void handleField(FieldDeclaration node, ICompilationUnit unit,
			 CompilationUnit cUnit, ClassObject targetClass) {
		if (!node.getType().isPrimitiveType()) {
			if (node.getType().isArrayType() && ((ArrayType) node.getType()).getComponentType().isPrimitiveType())
				return;
...
//					isRefactorable = false;
	...
	protected void handleMethod(MethodDeclaration node, ICompilationUnit unit,
		...
		if(!node.isConstructor()) {
			handleReturnType(node, unit);
		}
		handleParameters(node,unit);
	}

	private void handleParameters(MethodDeclaration node, ICompilationUnit unit) {
	...
	private void handleReturnType(MethodDeclaration node, ICompilationUnit unit) {
...
//					isRefactorable = false;
	...
	private ITypeBinding getMostGeneralType(Collection/*<ITypeBinding>*/ types) {
		...
		int mostGeneralTypeIndex = types.size() - 1;
//		ITypeBinding type =  (ITypeBinding) types.toArray()[mostGeneralTypeIndex];
