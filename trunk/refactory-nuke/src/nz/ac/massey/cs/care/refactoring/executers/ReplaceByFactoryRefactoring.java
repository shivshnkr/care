...
//		the next two for loops are for reflection for static members
...
//			writeDownCodeChanges(sourceRewriter);
	...
	private void addRequiredTargetImportDeclarations(MethodDeclaration method, ImportRewrite importRewrite) {
		Type s = method.getReturnType2();
		ITypeBinding tb = s.resolveBinding();
		if(tb==null){
	...
	private void createInitMethodInFactory(ASTNode instance, String methodName, RefactoringStatus status) {
		...
		ImportRewrite imports = ImportRewrite.create(factoryCompilationUnit, true);
			...
			if(targetSuperclassName != null) {
				...
				try {
					if(targetClass.getSuperclass() !=null) {
						superQualifiedName = targetClass.getSuperclass().getClassType();
					}
					...
					method.setReturnType2(methodast.newSimpleType(methodast.newName(superQualifiedName)));
...
//		imports.addImport(factoryMethodReturnType);
...
//			sourceRewriter.set(returnStatement, ReturnStatement.EXPRESSION_PROPERTY, returnInstance, null);
		...
		addRequiredTargetImportDeclarations(method, imports);
		try {
			TextEdit targetImportEdit = imports.rewriteImports(new NullProgressMonitor());
			if(imports.getCreatedImports().length > 0) {
				multiTextEdit.addChild(targetImportEdit);
				factoryCompilationUnitChange.addTextEditGroup(new TextEditGroup("Add required import declarations", new TextEdit[] {targetImportEdit}));
			}
		}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException,
		...
		try {
			...
			CompositeChange changes = new CompositeChange("Introduce Factory Methods");
