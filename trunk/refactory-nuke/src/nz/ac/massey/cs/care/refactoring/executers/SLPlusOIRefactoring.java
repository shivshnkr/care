...
public class SLPlusOIRefactoring extends Refactoring {
	...
	public SLPlusOIRefactoring(Edge winner) {
		this.winner = winner;
	}

	@Override
	public String getName() {
		return "SL+OI Refactoring";
	}
...
//			int numOfDeclarations =  visitor.getNumberOfDeclarationElements();
			...
			//precondition 1
			if(numOfOtherUses > 0 ) { //it means we have two dependency types CI + SMI
				if(visitor.isRefactorable()){
					...
					if(!status.hasError()) {
						refactoring1 = new OIRefactoring(winner);
						status.merge(refactoring1.checkAllConditions(new NullProgressMonitor()));
					}
	...
	public int getNoOfDeclarationElements() {
		return 0;//TODO visitor.getNumberOfDeclarationElements();
	}
	
	public int getNoOfDICases() {
		return 0; //TODO:  visitor.getNumberOfOtherUses();
