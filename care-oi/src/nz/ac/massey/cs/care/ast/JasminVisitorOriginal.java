 ...
 * JASMIN</A> format.
 *
 * @version $Id: JasminVisitor.java,v 1.10 2001/08/15 14:55:42 dahm Exp $
 * @author  <A HREF="http://www.berlin.de/~markus.dahm/">M. Dahm</A>
 */
public class JasminVisitorOriginal extends EmptyVisitor  {
  private JavaClass       clazz;
  private PrintWriter     out;
  private String          class_name;
  private ConstantPoolGen cp;

  public JasminVisitorOriginal(JavaClass clazz, OutputStream out) {
    this.clazz = clazz;
  ...
  public static void main(String[] argv) { 
	...
	for(int i=0; i < argv.length; i++) {
	  ...
	  new JasminVisitorOriginal(java_class, out).disassemble();
	}
      }	  
    } catch(Exception e) {
      e.printStackTrace();
